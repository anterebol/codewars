// link: https://www.codewars.com/kata/58ad388555bf4c80e800001e/train/javascript
// Task
...
function cutTheRopes(ropes) {
  const result = [];
  while (ropes.reduce((acc, rope) => acc + rope, 0) > 0) {
    const counter = ropes.reduce((a, b) => a + (b ? 1 : 0), 0);
    result.push(counter);
    const short = Math.min(...ropes.filter((item) => item));
    ropes = ropes.map((item) => {
      const r = item - short;
      return r > 0 ? r : 0;
    });
  }
