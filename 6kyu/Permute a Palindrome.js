// link: https://www.codewars.com/kata/58ae6ae22c3aaafc58000079/train/javascript
...
function permuteAPalindrome(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    for (let k = 0; k < input.length; k++) {
      letterCounter += input[i] === input[k] ? 1 : 0;
    }
    obj[letter] = letterCounter;
  }
  for (const letter in obj) {
    unpaired += obj[letter] % 2 == 0 ? 0 : 1;
  }
  return unpaired <= 1 ? true : false;
}
